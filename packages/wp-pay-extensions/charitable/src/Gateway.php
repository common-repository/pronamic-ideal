...
class Gateway extends Charitable_Gateway {
	...
	public function __construct() {
		...
		$this->defaults = [
			'label' => PaymentMethods::get_name( $this->payment_method, \__( 'Pronamic', 'pronamic-ideal' ) ),
		];
	...
	public function gateway_settings( $settings ) {
		...
		$settings['transaction_description'] = [
			'type'     => 'text',
			'title'    => __( 'Transaction description', 'pronamic-ideal' ),
			'priority' => 8,
			'default'  => __( 'Charitable donation {donation_id}', 'pronamic-ideal' ),
			/* translators: %s: <code>{tag}</code> */
			'help'     => sprintf( __( 'Available tags: %s', 'pronamic-ideal' ), sprintf( '<code>%s</code> <code>%s</code> <code>%s</code>', '{donation_id}', '{first_campaign_name}', '{campaign_name}' ) ),
		];
